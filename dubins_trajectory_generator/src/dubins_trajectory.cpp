...
DubinsTrajectory::DubinsTrajectory(ros::NodeHandle &nh,
  ...
  publish_trajectory_server_ = nh_.advertiseService(
      "command", &DubinsTrajectory::commandServiceCallback, this);
  publish_trajectory_client_ = nh_.serviceClient<std_srvs::Trigger>("command");

  DubinsTrajectory::loadParams(nh_private);

  arc_angle_ =
      M_PI / 2 - atan((right_pylon_.x() - left_pylon_.x()) /
                      (right_pylon_.y() -
                       left_pylon_.y())); // Determines the angle our MAV turns
                                          // in one single curved part.

  interpylon_distance_ =
      (left_pylon_ - right_pylon_).norm(); // Distance between the two pylons
...
void DubinsTrajectory::computeTangencyPoints() {
  ...
  // I got the expression below just by solving a quadratic equation in
  // cos(theta) while calculating distance between loop_entry_point_ and
  // launch_pos_ using coordinate geometry. The expression was as below (0.0 has
  // been used at some places just because launch position is on x axis and
  // left_pylon_ is on the y axis.): (start_x-0.0)*(start_x-0.0) + (left_y -
  // 0.0)*(left_y - 0.0) - r*r = (r*sin(tangency_angle_) +
  // a)*(r*sin(tangency_angle_) + a) + (left_y - r*cos(tangency_angle_))*(left_y
  //- r*cos(tangency_angle_)).
  tangency_angle_ = acos(
      (left_y * r + sqrt(pow(start_x, 4) + start_x * start_x * left_y * left_y -
                         r * r * start_x * start_x)) /
      (left_y * left_y + start_x * start_x));

  loop_entry_point_.position.x() = arc_radius_ * sin(tangency_angle_);
  loop_entry_point_.position.y() =
      left_pylon_.y() - arc_radius_ * cos(tangency_angle_);
...
void DubinsTrajectory::computeFirstHalfLoop(uint lap_number) {
  ...
  Point start(left_pylon_.x(), left_pylon_.y() - arc_radius_, left_pylon_.z(),
              M_PI);
  Point end(right_pylon_.x(), right_pylon_.y() + arc_radius_, right_pylon_.z(),
            0.0);
  ...
  double angle_to_move =
      M_PI - tangency_angle_ -
      arc_angle_; // Determines how much angle still needs to be turned before
  ...
  while (prev_pos.yaw >= angle_to_move) {
    ...
    curr.addConstraint(mav_trajectory_generation::derivative_order::POSITION,
                       Eigen::Vector3d(curr_pos.position.x(),
                                       curr_pos.position.y(),
                                       curr_pos.position.z()));
  ...
  while (distance <= interpylon_distance_) {
    ...
    curr_pos.position.x() =
        prev_pos.position.x() + delta_distance_ * cos(angle_to_move);
    curr_pos.position.z() = start.position.z();
...
void DubinsTrajectory::computeSecondHalfLoop(uint lap_number) {
  ...
  Point start(right_pylon_.x(), right_pylon_.y() + arc_radius_,
              right_pylon_.z(), M_PI);
  Point end(left_pylon_.x(), left_pylon_.y() - arc_radius_, left_pylon_.z(),
            0.0);
  ...
  tangency_angle_ =
      0.0; // Re-calculated just because when the lap_number is 1 this value is
           // changed in the function computeFirstHalfLoop.
  arc_angle_ = M_PI / 2 -
               atan((right_pylon_.x() - left_pylon_.x()) /
                    (right_pylon_.y() -
                     left_pylon_.y())); // Re-calculated just because when the
...
void DubinsTrajectory::computePoints() {
  ...
  start.makeStartOrEnd(
      Eigen::Vector3d(launch_pos_.x(), launch_pos_.y(), launch_pos_.z()),
  ...
  hunter_killer.addConstraint(
      mav_trajectory_generation::derivative_order::POSITION,
      Eigen::Vector3d(hunter_killer_.x(), hunter_killer_.y(),
                      hunter_killer_.z()));
...
void DubinsTrajectory::generateTrajectory() {
  ...
  segment_times_ = mav_trajectory_generation::estimateSegmentTimes(
      vertices_, v_max_, a_max_);
  ...
  mav_trajectory_generation::drawMavTrajectory(trajectory_, distance_, frame_id,
                                               &markers_);
}

bool DubinsTrajectory::commandServiceCallback(
  ...
  mav_trajectory_generation::sampleWholeTrajectory(trajectory_, 0.1,
                                                   &trajectory_points);
  mav_msgs::msgMultiDofJointTrajectoryFromEigen(trajectory_points,
